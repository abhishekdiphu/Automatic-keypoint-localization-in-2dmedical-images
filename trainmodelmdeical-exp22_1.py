...
from datasets.legmedical import HANDXRAY             # -------------------------> Normal images 
#from datasets.legmedical_data_argum import HANDXRAY #----------------------> part images + normal Images 
...
parser.add_argument('--save_every', type=int, default=4000, help='frequency of saving the model')
...
#modelpath = torch.load('train-model-19/supervised-medical-660-lr-002/new_exp51_batchsize1/model_61_450.pt')
...
def evaluate_model(args, epoch, val_loader, fast_device, generator_model, discriminator_model): #val_loader
    ...
    for i, data in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            cur_gen_loss_dic_val = gen_single_loss(ground_truth, outputs, discriminator_model, mode=args.loss)
 #           cur_disc_loss_dic = disc_single_loss(ground_truth, outputs, discriminator_model)

            cur_gen_loss_val = cur_gen_loss_dic_val['loss']
 #          cur_disc_loss = cur_disc_loss_dic['loss']

            gen_loss_val += cur_gen_loss_val
...
#       val_disc_losses.append(val_disc_loss)
        with open(os.path.join(args.modelName, 'val_stats.bin'), 'wb') as f:
                pickle.dump((val_gen_losses), f)
...
#        disc_losses.append(float(loss_conf))
        ...
        if i % args.print_every == 0:
            ...
            plt.savefig('trainingImages/train_output_exp17.png')
            
            
            plt.clf()
            plt.imshow(np.sum(ground_truth['heatmaps'].detach().cpu().numpy()[0], axis=0))
            plt.savefig('trainingImages/train_gt_exp17.png')

            plt.clf()
            print("image shape_exp16: ", images.shape)
            plt.imshow((images.detach().cpu().numpy()[0].transpose(1, 2, 0) * 256).astype(np.uint8)) ## originally it was there , replaced by below line
            #plt.imshow((images.detach().cpu().numpy()[0].transpose(1, 2, 0) * 255)
            plt.savefig('trainingImages/train_img_exp17.png')
        # Save model
        ######################################################################################################################
        if i > 0 and i % 450 == 0:
        ...
        if i % args.validate_every == 0:
         ...
         #          cur_disc_loss_dic = disc_single_loss(ground_truth, outputs, discriminator_model)
                    ...
                    val_gen_losses.append(float(cur_gen_loss_val))
