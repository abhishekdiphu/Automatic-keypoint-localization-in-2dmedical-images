...
from discriminator02 import DiscPoseNet_SIMG as Discriminator2
...
#modelpath_g = torch.load('train-model-19/supervised-medical-660-lr-0002/batchsize8-experiment09/model_23_60.pt')
#generator_model = modelpath_g['generator_model']
#print(generator_model)
#print(generator_model)
#print(discriminator_model)

#modelpath_d = torch.load('train-model-31/pretrained_pose_sh_glass/experiment-07/pretrained_conf_pose_76_500.pt')
...
#   epoch_disc_loss = 0.0
    ...
    for i, data in enumerate(train_loader):
        ...
        if (args.use_gpu):
            images = images.to(fast_device)
            small_images = small_images.to(fast_device)
            ground_truth['heatmaps'] = ground_truth['heatmaps'].to(fast_device)
            ground_truth['occlusions'] = ground_truth['occlusions'].to(fast_device)
...
#        print("---------------------pose Discriminator training-------------------------------------------------")
        ...
        _ , _, p_fake = pck.StackedHourGlass(pck_out, ground_truth['heatmaps'])
         ...
         #cur_gen_loss_dic = gen_single_loss(ground_truth, outputs, discriminator_model, mode=args.loss)
        cur_disc_loss_dic = disc_loss_pose(ground_truth, outputs, small_images , discriminator_model_pose, detach=False, p_fake = p_fake )
...
#            cur_disc_loss_dic_conf += get_loss_disc_bce_with_logits(output, discriminator_model_conf, real=True, 
#                                                                    ground_truth =ground_truth['heatmaps'])
            cur_disc_loss_dic_pose +=  - get_loss_disc_bce_with_logits(torch.cat([output, small_images], 1), discriminator_model_pose, real=False,\
        ...
        loss = cur_gen_loss     + (cur_disc_loss_dic_pose)/138.0   #/220.0      #138.0      #/220. 
        ...
        if i > 0 and i % 100 == 0:
            ...
            torch.save({'generator_model': generator_model, 
                        ...
                        #'optim_disc_conf': optim_disc_conf,
                        #'optim_disc_pose': optim_disc_pose }, \
                         os.path.join(args.modelName, 'modeladversarial_{}_{}.pt'.format(epoch, i)))
