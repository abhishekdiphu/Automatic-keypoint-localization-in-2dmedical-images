...
parser.add_argument('--validate_every', type=int, default=500, help='frequency of evaluating on validation set')
...
#        conf_real.append(float(loss_conf_real))
        ...
        with open(os.path.join(args.modelName, 'stats.bin'), 'wb') as f:
            pickle.dump((gen_losses, disc_losses, pose_real,pose_fake ), f)                  ##disc_losses,, val_disc_losses, val_gen_losses
        ...
        print("Train iter: %d, generator loss : %f, pose discriminator loss : %f" % (i ,gen_losses[-1], disc_losses[-1]))
...
#                        'optim_disc_conf': optim_disc_conf,
                        'optim_disc_pose': optim_disc_pose }, \
                         os.path.join(args.modelName, 'pretrained_conf_pose_{}_{}.pt'.format(epoch, i)))
