...
modelpath_d = torch.load('train-model-31/pretrained_pose_sh_glass/experiment-07/pretrained_conf_pose_70_500.pt')
...
modelpath_c = torch.load('train-model-31/pretrained_conf_sh_glass/experient01/pretrained_conf_70_500.pt')
discriminator_model_conf = modelpath_c['discriminator_model_conf']
#print(generator_model)
#print(generator_model)
#print(discriminator_model)
...
#        print("outputs  for the discriminators :",len(outputs))
        ...
        loss = cur_gen_loss     + (cur_disc_loss_dic_pose)/138.0 +  cur_disc_loss_dic_conf/220.0
        ...
        with open(os.path.join(args.modelName, 'stats.bin'), 'wb') as f:
            pickle.dump((gen_losses, disc_losses, disc_conf_losses), f)                  ##disc_losses,, val_disc_losses, val_gen_losses
        ...
        print("Train iter: %d, generator loss : %f, pose discriminator loss : %f,  conf iscriminator loss : %f" % (i ,gen_losses[-1], disc_losses[-1], disc_conf_losses[-1]))
        ...
        if i % args.print_every == 0:
            ...
            plt.savefig('trainingImages/train_output_exp23-pretrained.png')
            
            
            plt.clf()
            plt.imshow(np.sum(ground_truth['heatmaps'].detach().cpu().numpy()[0], axis=0))
            plt.savefig('trainingImages/train_gt_exp23-pretrained.png')
            ...
            plt.savefig('trainingImages/train_img_exp16.png')
        # Save model
        ######################################################################################################################
        if i > 0 and i % 520 == 0:
            ...
            torch.save({'generator_model': generator_model, 
                        ...
                        'optim_disc_conf': optim_disc_conf,
                        'optim_disc_pose': optim_disc_pose }, \
                         os.path.join(args.modelName, 'pretrained-adversarial_conf_pose_{}_{}.pt'.format(epoch, i)))
