...
from losses import gen_single_loss, disc_single_loss, get_loss_disc_bce_with_logits, disc_loss_pose
import metrics
...
parser.add_argument('--save_every', type=int, default=11, help='frequency of saving the model')
...
parser.add_argument('--validation_sample_size', type=int, default=100, help='size of validation sample')
parser.add_argument('--validate_every', type=int, default=450, help='frequency of evaluating on validation set')
...
#train-model-33adversarial_models_pose_pretrainedexperiment11_alpha_138
modelpath_g = torch.load('train-model-19/supervised-medical-660-lr-002/new_exp51_batchsize1/model_61_450.pt')
generator_model = modelpath_g['generator_model']
#print(generator_model)
#print(generator_model)
#print(discriminator_model)

modelpath_d = torch.load('train-model-31/pretraining_discriminator/experiment60_pretrain_pose/pretrained_conf_pose_69_450.pt')
...
train_loader = torch.utils.data.DataLoader(lsp_train_dataset, batch_size=args.batch_size, num_workers = 1 ,  shuffle=False)
val_save_loader = torch.utils.data.DataLoader(lsp_val_dataset, batch_size=args.val_batch_size)
val_eval_loader = torch.utils.data.DataLoader(lsp_val_dataset, batch_size=args.val_batch_size, num_workers =1,  shuffle=False)
...
#                    'optim_disc_conf': optim_disc_conf,
                    ...
                    os.path.join(args.modelName, 'adversarial_model_' + str(epoch) + '.pt'))
...
#        print("---------------------pose Discriminator training-------------------------------------------------")
        ...
        _ , _, p_fake = pck.StackedHourGlass(pck_out, ground_truth['heatmaps'])
         ...
         #cur_gen_loss_dic = gen_single_loss(ground_truth, outputs, discriminator_model, mode=args.loss)
        cur_disc_loss_dic = disc_loss_pose(ground_truth, outputs, images , discriminator_model_pose, detach=False, p_fake = p_fake )
...
#                                                                    ground_truth =ground_truth['heatmaps'])
        ...
        loss = cur_gen_loss     + (cur_disc_loss_dic_pose)/180  # for pose discriminator = 180  , # for confidence discriminator = 220 
        ...
        with open(os.path.join(args.modelName, 'stats.bin'), 'wb') as f:
            pickle.dump((gen_losses, disc_losses), f)                  ##disc_losses,, val_disc_losses, val_gen_losses
        ...
        if i % args.print_every == 0:
            ...
            plt.clf()
            plt.imshow(np.sum(outputs[-1].detach().cpu().numpy()[0][0 : 6], axis=0))
            plt.savefig('trainingImages/train_output_exp11-pretrained.png')
            
            
            plt.clf()
            plt.imshow(np.sum(ground_truth['heatmaps'].detach().cpu().numpy()[0], axis=0))
            plt.savefig('trainingImages/train_gt_exp11-pretrained.png')
            ...
            plt.savefig('trainingImages/train_img_exp11.png')
        # Save model
        ######################################################################################################################
        if i > 0 and i % 500 == 0:
            ...
            torch.save({'generator_model': generator_model, 
                        ...
                        #'optim_disc_conf': optim_disc_conf,
                        'optim_disc_pose': optim_disc_pose }, \
                         os.path.join(args.modelName, 'modeladversarial_pose_{}_{}.pt'.format(epoch, i)))
        ...
        if i % args.validate_every == 0:
            for j, data in enumerate(val_eval_loader):
                ...
                with torch.no_grad():
                    ...
                    cur_gen_loss_val = gen_single_loss(ground_truth, outputs, None, mode=args.loss)
                    cur_gen_loss_val = cur_gen_loss_val['loss']
                    val_gen_losses.append(float(cur_gen_loss_val.item()))
                    print("validation loss :" , val_gen_losses[-1])
                    
                    with open(os.path.join(args.modelName, 'validationloss.bin'), 'wb') as p:
                         pickle.dump((val_gen_losses), p)
